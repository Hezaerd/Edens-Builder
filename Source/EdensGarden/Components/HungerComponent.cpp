...
{
	Super::BeginPlay();
	Object = Cast<ABasicObject>(GetOwner());
	// ...
...
void UHungerComponent::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	if (Object)
	{
		if(Object->CurrentHunger >= 0)
			Object->CurrentHunger -= Object->HungerFactor * DeltaSeconds;
	}

	if (Object->CurrentHunger <= 0)
	{
		Object->Health -= Object->HungerFactor * DeltaSeconds;
	}
	// ...
}
